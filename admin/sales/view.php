...
if (!$sale_id) {
    set_flash_message('error', 'Invalid sale.');
...
if (!$sale_result || mysqli_num_rows($sale_result) == 0) {
    set_flash_message('error', 'Sale not found.');
    redirect('admin/sales/index.php');
}
...
$pay_query = "SELECT p.*, 'link' AS source FROM payments p WHERE p.sale_id = $sale_id OR (p.order_id IS NOT NULL AND p.order_id = " . ($sale['order_id'] ?? 0) . ")";
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            <?php if ($sale['status'] !== 'cancelled'): ?>
                ...
                <?php if ($user_role == ROLE_ADMIN && $sale['status'] == 'pending'): ?>
