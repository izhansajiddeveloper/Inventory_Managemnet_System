...
if (!empty($search)) {
    $query .= " AND (s.id LIKE '%$search%' OR c.name LIKE '%$search%')";
}
if (!empty($status_filter)) {
    $query .= " AND s.status = '$status_filter'";
}
if (!empty($date_filter)) {
    $query .= " AND DATE(s.created_at) = '$date_filter'";
}
...
<style>
    :root {
        --card-shadow: 0 10px 40px -15px rgba(0, 0, 0, .1);
    ...
    .btn-new:hover {
        ...
        box-shadow: 0 10px 24px -8px #2563eb;
        color: #fff;
    }
