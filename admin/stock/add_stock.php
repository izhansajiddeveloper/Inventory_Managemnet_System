...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_id = isset($_SESSION[SESSION_USER_ID]) ? $_SESSION[SESSION_USER_ID] : NULL;
    ...
    if (!$product_id || $quantity <= 0) {
        ...
        try {
            ...
            $check_stmt = $pdo->prepare("SELECT id FROM product_stock WHERE product_id = ?");
            $check_stmt->execute([$product_id]);
            $existing_stock = $check_stmt->fetch();

            if ($existing_stock) {
                $update_stmt = $pdo->prepare("UPDATE product_stock SET quantity = quantity + ?, updated_at = NOW() WHERE product_id = ?");
                $update_stmt->execute([$quantity, $product_id]);
            } else {
                $insert_stmt = $pdo->prepare("INSERT INTO product_stock (product_id, quantity, created_at) VALUES (?, ?, NOW())");
                $insert_stmt->execute([$product_id, $quantity]);
            }

            // 2. Insert into product_transactions
            $trans_stmt = $pdo->prepare("INSERT INTO product_transactions (product_id, type, quantity, reference_type, note, created_by, created_at) VALUES (?, 'IN', ?, 'manual', ?, ?, NOW())");
            $trans_stmt->execute([$product_id, $quantity, $note, $user_id]);
...
<style>
    .form-card {
        ...
        box-shadow: 0 10px 40px -15px rgba(0,0,0,0.08); overflow: hidden;
        max-width: 650px; margin: 0 auto;
    }
    .form-header { padding: 2rem; background: #fafbff; border-bottom: 1px solid #f1f5f9; }
    .form-body { padding: 2.5rem; }
    .form-label { font-size: 0.85rem; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 0.6rem; display: block; }
    .form-control { 
        height: 52px; border-radius: 14px; border: 1.5px solid #e2e8f0; 
        font-size: 0.95rem; transition: all 0.2s; padding: 0 1.2rem;
    }
    .form-control:focus { outline: none; border-color: #2563eb; box-shadow: 0 0 0 4px rgba(37, 99, 235, 0.1); }
    .btn-submit { 
        height: 52px; background: #2563eb; color: white; border: none; font-weight: 700;
        border-radius: 14px; transition: all 0.2s; display: inline-flex; align-items: center; justify-content: center; gap: 0.75rem;
    }
    .btn-submit:hover { background: #1d4ed8; transform: translateY(-2px); box-shadow: 0 10px 25px -10px #2563eb; }
    .back-btn { 
        color: #64748b; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem;
        background: white; padding: 0.5rem 1.2rem; border-radius: 12px; border: 1px solid #e2e8f0; transition: all 0.2s;
    }
    .back-btn:hover { color: #2563eb; border-color: #2563eb; background: #f8fafc; }
</style>
